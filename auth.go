...
// group and role as well as a hash of their username and password. When creating
...
type AuthBackend interface {
	...
	User(username, group string) (user UserData, e error)
	Users() (users []UserData, e error)
	DeleteUser(username, group string) error
	Close()
}
...
func (a *Authorizer) AddGroup(group string) {
	a.groups[group] = true
}
...
func (a Authorizer) Login(rw http.ResponseWriter, req *http.Request, u string, g string, p string, dest string) error {
	session, _ := a.cookiejar.Get(req, "auth")
	if session.Values["username"] != nil {
		return mkerror("already authenticated")
	}
	if user, err := a.backend.User(u, g); err == nil {
...
func (a Authorizer) Register(rw http.ResponseWriter, req *http.Request, user UserData, password string) error {
	...
	// Validate group
	if _, ok := a.groups[user.Group]; !ok {
		return mkerror("non-existant group")
	}

	// Validate username in group
	_, err := a.backend.User(user.Username, user.Group)
	if err == nil {
	...
	hash, err := bcrypt.GenerateFromPassword([]byte(user.Username+user.Group+password), 8)
	if err != nil {
...
func (a Authorizer) Update(rw http.ResponseWriter, req *http.Request, p string, e string) error {
	...
	group, ok := authSession.Values["group"].(string)
	if !ok {
		return mkerror("not logged in")
	}
	user, err := a.backend.User(username, group)
	...
	newuser := UserData{username, group, email, hash, user.Role}
...
func (a Authorizer) Authorize(rw http.ResponseWriter, req *http.Request, redirectWithMessage bool) error {
	...
	group := authSession.Values["group"]
	if !authSession.IsNew && username != nil && group != nil {
		_, err := a.backend.User(username.(string), group.(string))
		if err == ErrMissingUser {
...
func (a Authorizer) AuthorizeRole(rw http.ResponseWriter, req *http.Request, role string, redirectWithMessage bool) error {
	...
	username := authSession.Values["username"]
	group := authSession.Values["group"]
	if user, err := a.backend.User(username.(string), group.(string)); err == nil {
...
func (a Authorizer) AuthorizeGroup(rw http.ResponseWriter, req *http.Request, group string, redirectWithMessage bool) error {
	if _, ok := a.groups[group]; !ok {
		return mkerror("no such group")
	}
	...
	username := authSession.Values["username"].(string)
	g := authSession.Values["group"].(string)
	if group != g {
		return mkerror("usre belongs to wrong group")
	}
	_, err = a.backend.User(username, group)
	return err
}
...
func (a Authorizer) CurrentUser(rw http.ResponseWriter, req *http.Request) (user UserData, e error) {
	...
	group, ok := authSession.Values["group"].(string)
	if !ok {
		return user, mkerror("Group found in authsession")
	}

	return a.backend.User(username, group)
}
...
func (a Authorizer) DeleteUser(username, group string) error {
	err := a.backend.DeleteUser(username, group)
