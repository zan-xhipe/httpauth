...
func NewSqlAuthBackend(driverName, dataSourceName string) (b SqlAuthBackend, e error) {
	...
	b.db = db
	_, err = db.Exec(`create table if not exists goauth (Id int not null auto_increment, Username varchar(255), Groupname varchar(255), Email varchar(255), Hash varchar(255), Role varchar(255), primary key (Id))`)
	if err != nil {
		return b, mksqlerror(err.Error())
	}
	...
	if driverName == "postgres" {
		b.userStmt, err = db.Prepare(`select Email, Hash, Role from goauth where Username = $1 and Groupname = $2`)
		if err != nil {
			return b, mksqlerror(fmt.Sprintf("userstmt: %v", err))
		}
		b.usersStmt, err = db.Prepare(`select Username, Groupname, Email, Hash, Role from goauth`)
		if err != nil {
			return b, mksqlerror(fmt.Sprintf("usersstmt: %v", err))
		}
		b.insertStmt, err = db.Prepare(`insert into goauth (Username, Groupname, Email, Hash, Role) values ($1, $2, $3, $4, $5)`)
		if err != nil {
			return b, mksqlerror(fmt.Sprintf("insertstmt: %v", err))
		}
		b.updateStmt, err = db.Prepare(`update goauth set Email = $1, Hash = $2, Role = $3 where Username = $4 and Groupname = $5`)
		if err != nil {
			return b, mksqlerror(fmt.Sprintf("updatestmt: %v", err))
		}
		b.deleteStmt, err = db.Prepare(`delete from goauth where Username = $1, and Groupname = $2`)
		if err != nil {
			return b, mksqlerror(fmt.Sprintf("deletestmt: %v", err))
		}
	} else {
		b.userStmt, err = db.Prepare(`select Email, Hash, Role from goauth where Username = ? and Groupname = ?`)
		if err != nil {
			return b, mksqlerror(fmt.Sprintf("userstmt: %v", err))
		}
		b.usersStmt, err = db.Prepare(`select Username, Groupname, Email, Hash, Role from goauth`)
		...
		b.insertStmt, err = db.Prepare(`insert into goauth (Username, Groupname, Email, Hash, Role) values (?, ?, ?, ?, ?)`)
		if err != nil {
			return b, mksqlerror(fmt.Sprintf("insertstmt: %v", err))
		}
		b.updateStmt, err = db.Prepare(`update goauth set Email = ?, Hash = ?, Role = ? where Username = ? and Groupname = ?`)
		if err != nil {
			return b, mksqlerror(fmt.Sprintf("updatestmt: %v", err))
		}
		b.deleteStmt, err = db.Prepare(`delete from goauth where Username = ? and Groupname = ?`)
		if err != nil {
			return b, mksqlerror(fmt.Sprintf("deletestmt: %v", err))
		}
	}
...
func (b SqlAuthBackend) User(username, group string) (user UserData, e error) {
	row := b.userStmt.QueryRow(username, group)
...
func (b SqlAuthBackend) Users() (us []UserData, e error) {
	...
	var (
		username, group, email, role string
		hash                         []byte
	)
	for rows.Next() {
		err = rows.Scan(&username, &group, &email, &hash, &role)
		if err != nil {
			return us, mksqlerror(err.Error())
		}
		us = append(us, UserData{username, group, email, hash, role})
	}
	return us, nil
}
...
func (b SqlAuthBackend) SaveUser(user UserData) (err error) {
	if _, err := b.User(user.Username, user.Group); err == nil {
		_, err = b.updateStmt.Exec(user.Email, user.Hash, user.Role, user.Username, user.Group)
		if err != nil {
			return err
		}
	} else {
		_, err = b.insertStmt.Exec(user.Username, user.Group, user.Email, user.Hash, user.Role)
		if err != nil {
			return err
		}
	}
...
func (b SqlAuthBackend) DeleteUser(username, group string) error {
	result, err := b.deleteStmt.Exec(username, group)
	if err != nil {
